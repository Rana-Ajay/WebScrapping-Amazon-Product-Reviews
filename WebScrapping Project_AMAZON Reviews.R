#Install pacman. It reduces typing in package management actions.
...
#Get the product code from Amazon URl
prod_code = "B082X1HRV5"

#Build Amazon's URL by adding product code at the end
...
#Following function parses the data and gives the following values - prod, title, author, date, ver.purchase, format, stars, comments and helpful for each product
amazon_scraper <- function(doc, reviewer = T, delay = 0){
  ...
  if(reviewer == T){
    ...
    #average rating of past reviews
...
#Sentimental analysis of these first 100 pages of reviews/comments.
...
#Create a subset of the top 5 reviews as 'best_reviews' based on the sentiment score
best_reviews <- slice(reviews_all, top_n(ordered_sent_agg, 5, ave_sentiment)$element_id)
best_reviews

#Highlight the top 5 reviews by sentiment polarity (positive = green; negative = pink) and present them in an html file.
highlight(sentiment_by(best_reviews$comments))
#with(best_reviews, sentiment_by(comments)) %>% highlight()

#Create a subset of the bottom 5 reviews as 'worst_reviews' based on their sentiment scores
worst_reviews <- slice(reviews_all, top_n(ordered_sent_agg,5, -ave_sentiment)$element_id)
worst_reviews
